...
true_pos = (0:N-1) * true_velocity; %(m)
...
hold off;
legend('Zmierzone Położenie','Prawdziwe Położenie', 'Ocenione Położenie', 'Location', 'southoutside', 'Orientation', 'horizontal');
xlabel('Krok Czasowy');
ylabel('Położenie');
...
hold on;
plot(1:N, true_velocity * ones(1, N), '--b', 'LineWidth', 1.5);
hold off;
legend('Oceniona Prędkość', 'Prawdziwa Prędkość', 'Location', 'southoutside', 'Orientation', 'horizontal');
xlabel('Krok Czasowy');
ylabel('Prędkość');
title('Prędkość');
sgtitle('Śledzony obiekt ze stałą prędkością, v = 100 m/s');
